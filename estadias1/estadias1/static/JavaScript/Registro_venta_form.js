...
document
  ...
  .addEventListener("click", function (event) {
    ...
    if (isNaN(cantidad) || isNaN(precioTotal)) {
      alert(
        "Por favor, introduce valores válidos para cantidad y precio total."
      );
      return;
    }
    ...
    const rfcId = document.querySelector(
      `#id_id_cliente option[value="${clienteId}"]`
    ).textContent;
  ...
  .addEventListener("click", function (event) {
    ...
    if (filaParaEliminar) {
      ...
      totalVenta.textContent = (
        parseFloat(totalVenta.textContent) - precioTotal
      ).toFixed(2);
